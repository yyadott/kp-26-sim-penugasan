...
async function main() {
  ...
  await prisma.user.create({
    data: { id: 3, nama: 'Budi Santoso, S.T., M.Si.', email: 'budi.santoso@dishub.go.id', password: 'password', departemen_id: deptKepeg.id, role_id: rolePegawai.id }
  });
  await prisma.user.create({
    data: { id: 4, nama: 'Siti Rahmawati, S.H.', email: 'siti.rahmawati@satpolpp.go.id', password: 'password', departemen_id: deptPM.id, role_id: rolePegawai.id }
  });
  await prisma.user.create({

...
export const DetailPegawaiPage = () => {
  ...
  const pegawai = dummyPegawaiList.find(p => p.id === id);
  ...
  const dummyActivities = [
    { id: 1, action: 'Mengunggah Laporan Tugas', task: 'Monitoring Jalan Provinsi', date: '12 Agustus 2026 10:30', status: 'Selesai' },
    { id: 2, action: 'Menerima Penugasan Baru', task: 'Pengawasan Lalu Lintas', date: '11 Agustus 2026 08:15', status: 'Baru' },
    { id: 3, action: 'Melakukan Presensi Datang', task: '-', date: '11 Agustus 2026 07:45', status: 'Hadir' },
    { id: 4, action: 'Mengubah Status Tugas', task: 'Penertiban PKL', date: '09 Agustus 2026 14:20', status: 'Proses' },
    { id: 5, action: 'Melakukan Presensi Pulang', task: '-', date: '08 Agustus 2026 16:30', status: 'Hadir' },
  ];
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col items-center text-center">
              ...
              <span className={`mt-3 inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ring-1 ring-inset ${
        ...
        <div className="md:col-span-2">
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden h-full flex flex-col">
            <div className="flex items-center gap-2 border-b border-slate-200 bg-slate-50/50 px-6 py-4">
              ...
              <h3 className="font-bold text-slate-800">Riwayat Aktifitas Terakhir</h3>
            </div>
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-white text-slate-600 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-6 py-3 font-semibold">Keterkaitan</th>
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {dummyActivities.map((activity) => (
                    <tr key={activity.id} className="hover:bg-slate-50/50 transition">
                      <td className="px-6 py-4 whitespace-nowrap text-slate-500">{activity.date}</td>
                      <td className="px-6 py-4 font-medium text-slate-800">{activity.action}</td>
                      <td className="px-6 py-4 text-slate-600">{activity.task}</td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${
                          activity.status === 'Selesai' || activity.status === 'Hadir' ? 'bg-emerald-50 text-emerald-700 ring-emerald-600/10' :
                          activity.status === 'Proses' ? 'bg-amber-50 text-amber-700 ring-amber-600/10' :
